...
fig2 %>% # Plotter fig2. 
  ...
  ggplot(aes(y=næring, x= prosent)) + theme_bw() + geom_col() + ggtitle("Sysselsetting etter næring (2021)", subtitle = "- Årsverk i prosent av total") + ylab("") + xlab("Figur 1.2")
```
...
```{r}
df2 %>% # Bruker som referanse til fig3. 
  mutate(value = value*1000) # Ganger ut for verdi. 

df_tidy %>% # Bruker som referanse til fig3.  
...
fig3 %>% # Plotter fig3. 
  ggplot(aes(x=næring,y=bruttosysselsatt)) + geom_col() + theme_minimal() + theme(axis.text.x = element_text(angle = 20,hjust = 1)) + ggtitle("Bruttoprodukt per sysselsatt i noen utvalgte næringer", subtitle = "- løpende 2021 priser") + xlab("Figur 1.3") + ylab("Kr")
```

Figur 1.3 viser bruttoproduksjon per sysselsatt i fire utvalgte næringer.
...
Bruttoproduksjon er verdiskapning, og for å skape verdier antas det at man ved å kombinere arbeidskraft og realkapital skaper noe som er verdt mer enn produktinnsatsen. Det blir slik et spørsmål om hvordan man måler verdiskapning. Om vi måler produktiviteten utifra profitt, så er ikke bruttoprodukt per sysselsatt en god måte å måle produktiviteten i offentlig sektor på. Det er store forskjeller mellom det private og det offentlige. I det private selges produktene på et marked (markedsrettet virksomhet), i det offentlige stammer inntekter fra bevilgning (ikke markedsrettet). I nasjonalregnskapet er bruttoproduksjon i offentlig sektor lik lønnskostnadene, i det private er dette gitt som salgsverdien minus produktinnsatsen. Verdiskapningen i det offentlige er således ikke avhengig av hvor mye verdier som igjennom overskudd, men av størrelsen på sektoren.
...
Produktprisene har en særlig stor innvirkning på bruttoproduksjon av olje og gass. Når bruttoproduksjon måles i løpende priser fjernes ikke virkningen av endringer i priser, og det er derfor vi ser de store svingningene i bruttoproduksjon. For å ser på den relative endringen bruker vi faste priser.

Olje og gass er volatile råvarer i hovedsak på grunn av endringer i tilbud og etterspørsel. Dersom etterspørselen økes over det tilbudet som er tilgjengelig vil dette føre til en økning i pris, og motsatt vil prisen falle dersom tilbudet er større enn det kvantum som etterspørres. \
Olje og gass etterspørselen er lite sensitiv for prisendringer (inelastisk) ettersom det på kort sikt er få substitusjonsmuligheter for etterspørrerne. Det er også en treghet i tilpasning ved økt etterspørsel fordi det er vanskelig for produsentene å skru opp produksjonen raskt nok. Olje og gass brukes også som innsatsfaktor i andre næringer som gjør at markedet er sårbart for konjunktursvingninger. I tillegg påvirkes tilbud og etterspørsel av klimaendringer og politiske handlinger som ofte er uforutsette eller vanskelig å påvirke. Alle disse faktorene er med på å gjøre olje og gass prisen mer volatil. I figur 3.3 har jeg regnet den gjennomsnittlige prisen til produsentene i olje og gass næringen, og det blir mer tydelig at bruttoproduksjonen i løpende priser overlapper med prisutviklingen i markedet.

I figur 3.2 ser vi at olje og gass næringen har hatt en økning i reel økonomisk vekst. Dette skyldes at næringen har blitt mer effektiv enn før, på grunn av bedre teknologi, bedre maskiner og utstyr (realkapital), og mer kompetanse.
...
Verdiskaping på fastlandet er all verdiskapning utenom olje, gass og utenriks sjøfart. Vi trekker fra olje og gass næringen på grunn av svingninger i pris, da får vi et mer presist mål for BNP.
